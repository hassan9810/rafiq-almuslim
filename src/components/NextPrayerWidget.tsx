...
export function NextPrayerWidget() {
  ...
  useEffect(() => {
    if (location) {
      setPrayerTimes(calculatePrayerTimes(location.latitude, location.longitude, new Date(), calculationMethod));
    }
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (next) setTimeUntilNext(getTimeUntilNextPrayer(next.time));
    };
    update();
